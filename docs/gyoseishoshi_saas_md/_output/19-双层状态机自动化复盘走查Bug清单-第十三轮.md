...
**统计**：24 条中 **20 条 ✅ PASS**（含 R12 §1 标 ✅ FIXED 的 12 条全部 reaffirm + R12 标 ❌ FAIL 但实测已 land 的 8 条「R12 land 摘要漏更新」），**1 条 ⚠️ MOSTLY PASS（BUG-149 → 增量 BUG-162）**，**1 条 ⚠️ FIX-LANDED-NO-DATA（BUG-140 → 关联 BUG-159）**，**1 条 ⚠️ MOSTLY PASS（BUG-142 → 关联 BUG-157）**，**1 条 ❌ FAIL（BUG-137）**，**1 条受阻未验（BUG-151，受 BUG-158 阻断）**。
...
| **BUG-159** | P2 [BE/FE] | 建案向导 Step 3 显示 `Group 继承自主申请人：东京一组`，但案件实际持久化 `cases.group_id = null`；下游 `BillingTable` row Group cell 三行均 fallback 到 `—`（BUG-140 fix 已 land 但永远 alias miss）；`/api/billing-plans` items[].groupId = null。 | 建案 service 在 `cases.service.ts#create` 路径未把"主申请人 → group"继承结果写入 `cases.group_id`；UI 端 inheritance 显示是 client-side computed，与 server-side 持久化分裂。|
| **BUG-160** | P2 [BE] | 直接 `POST /api/cases`（用合法 userId / customerId）返回 `500 Internal server error`，无具体错误信息可让前端 fallback；BMV 门禁应该返回 400 BadRequest，但实际是 unhandled exception。 | 推测在 `cases.service.ts#create` BMV gate 检查 / customer bmvProfile 解析路径中存在未捕获的 unhandled exception；进入 SQL `insert into cases` 之前已抛错。建议补 `try/catch` + `BadRequestException` 包装。|
...
| P1 | 1 | BUG-137（继承）|
| P2 | 5 | BUG-157 / 159 / 160 / 164 + BUG-149 增量 BUG-162（实际 P3）|
| P3 | 3 | BUG-161 / 162 / 163 |
| **本轮新增** | **8** | — |

### 0.5 三句话结论

1. **R12 §1 24 条修复整体大幅 land 到位**：`20/24 ✅ PASS`（含 R12 标 ❌ FAIL 但实测已 land 的 6 条「文档遗漏」），`1/24 ❌ FAIL`（BUG-137 仍卡 birthday=""），`3/24 ⚠️ MOSTLY PASS / FIX-NO-DATA`（BUG-140 / 142 / 149）。R12 文档对 BUG-133 / 138 / 139 / 143 / 144 / 152 的"❌ FAIL（继承）"结论需要回灌为「R13 实测确认 ✅ FIXED」。
...
3. **三处衍生数据偏差与一处 nav 缺漏**叠加：①BUG-159 建案 group_id 不持久化让 BUG-140 修复永远命中 disabled fallback；②BUG-161 / 163 建案/任务页直显客户与 case UUID；③BUG-160 直接 API 路径 BMV 门禁返回 500 而非 400；④BUG-157 sidebar 没有 Tasks 入口。R13 在 mempalace `prepare_grounded_answer` 引用的"经营管理签续签场景执行重点 = 持续经营 + 持续在留事实"维度看，这条 P0 让事务所连"建立续签案件"的初始动作都做不到。
...
### BUG-159 [P2][BE/FE] 建案 `cases.group_id` 不持久化（继承显示与持久化分裂，BUG-140 fix 永远 alias miss）
...
### BUG-160 [P2][BE] 直接 `POST /api/cases` 返回 500（应是 400 BadRequest）
...
#### BUG-140 ❌→⚠️ FIX-LANDED-NO-DATA

```js
// /#/billing ; en-US
table.tbody tr[].td[3] = "—"   × 3  // BUG-140 fix 已 land（resolveGroupLabel 调用 + disabledSuffix），
                                    // 但上游 cases.group_id = null（→ BUG-159），永远 alias miss
```
...
- **R13 守门补强**：
  ...
  - server `cases.service.ts#create` 加 `try/catch` + `BadRequestException`，禁止 unhandled exception 路径返回 500（BUG-160）；
  - server `cases.service.ts#create` 强制把 `customer.base_profile.group` 作为 `cases.group_id` 默认继承值（BUG-159）。
...
| Step 14（收费） | `/#/billing` | filter 下拉无 JA leftover（✅ BUG-134）；row Group 列全 `—`（⚠️ BUG-140 fix-no-data → BUG-159）。 | §2.1 / §2.2 |
...
├── ⚠️ MOSTLY PASS  × 3   (BUG-140 fix-no-data / BUG-142 nav 缺漏 / BUG-149 缩写 leftover)
...
# 5) BUG-159 confirm（建案 group_id 不持久化）
curl /api/cases?scope=all | jq '.items[0:3] | .[] | .groupId'   → all null ❌
curl /api/billing-plans   | jq '.items[0:3] | .[] | .groupId'   → all null ❌
...
# → 500 Internal server error  ❌ 应为 400 BadRequest（BMV gate）
```
...
- API：BMV 案件创建路径 `POST /api/cases`（BUG-160）与 BMV intake `PATCH /api/customers/<id>/bmv-intake`（缺 endpoint，404）是本轮最显眼的两处 server-side 缺口。
