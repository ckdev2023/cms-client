...
> **状态：已合并** — 本文件的内容已合并至 [SKILL-PROTOCOL.md](SKILL-PROTOCOL.md)。
>
> 新建 skill 请直接参考 `SKILL-PROTOCOL.md`。本文件保留仅为历史参考。
>
...
| v1.1 | 2026-04-10 | T12 统一：创建 [SKILL-PROTOCOL.md](SKILL-PROTOCOL.md) 为权威文件；本文件降级为历史参考 |

---

## 8. 现有 Skill 迁移参考

如需将现有 skill 迁移到本协议，对照以下映射表：

### prototype-module-split

| 协议章节 | 现有章节 | 迁移说明 |
|---------|---------|---------|
| Purpose | Purpose | 直接对应 |
| Triggers | frontmatter description | 扩展为独立节 |
| Required Inputs | Gold Reference | 重命名 |
| Deliverables | Required Deliverables | 重命名 |
| Workflow | Mandatory Workflow | 重命名 |
| Rules | Boundary Rules + Output Rules | 合并 |
| Anti-Patterns | Anti-Patterns | 直接对应 |
| References | Script, Data, Reference Usage | 重组为索引 |
| Completion | Completion | 直接对应 |

### cursor-task-orchestrator

| 协议章节 | 现有章节 | 迁移说明 |
|---------|---------|---------|
| Purpose | Overview + Core stance | 合并 |
| Triggers | Example triggers | 重命名 + 前移 |
| Required Inputs | — | 新增 |
| Deliverables | What to produce + Required output files | 合并 |
| Workflow | Mandatory workflow | 直接对应 |
| Rules | Hard rules + Dependency-first + Task document + Output behavior + Validation protocol | 合并为一节 + 子节 |
| Anti-Patterns | AI traps to actively prevent | 重命名 |
| References | References + Script generation guidance | 合并为索引 |
| Completion | Quality gate + Merge and review stage | 合并 |
