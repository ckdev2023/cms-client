...
- [prototype-split-orchestrator SKILL.md](../prototype-split-orchestrator/SKILL.md) — 流水线编排 skill（execute 完成后、spec sync 之前可按需触发本 skill）
- [split-manifest.schema.json](../prototype-module-split/data/split-manifest.schema.json) — split-manifest 权威 JSON Schema（`sharedCandidates` 列出了拆分 skill 识别的共享提取候选项）
...
- **触发条件**：模块有重复壳层/样式，且 `shared/` 版本缺失或过时
- **输入参照**：`split-manifest.json` 的 `sharedCandidates`（styles/shell/scripts）列出了拆分 skill 识别的共享提取候选——本 skill 以此为起点，不重复扫描
