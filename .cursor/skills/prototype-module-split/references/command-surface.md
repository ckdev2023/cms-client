...
Six project-level commands, each scoped to one pipeline phase.
...
## 6. `prototype-split-closeout`

**Phase**: closeout
**Purpose**: Align `split-manifest.json`, synced page specs, and `regression-checklist.json` after regression artifacts are authored.
...
```bash
# 1. Verify execute completed and closeout prerequisites exist
node .cursor/skills/prototype-module-split/scripts/phase-gate.mjs \
  --module-dir <path> --target-phase closeout [--suffix <suffix>]

# 2. Re-validate split-manifest after syncing lightweight regression refs
node .cursor/skills/prototype-module-split/scripts/validate-manifest.mjs \
  --manifest <module-dir>/split-manifest[-suffix].json --check-provenance

# 3. Validate machine-readable regression checklist
node .cursor/skills/prototype-module-split/scripts/validate-manifest.mjs \
  --manifest <module-dir>/regression-checklist[-suffix].json --type regression-checklist --check-provenance
```

### Output

- Updated `split-manifest[-suffix].json` with closeout-aligned `regressionChecklist` refs and phase notes
- Validated `regression-checklist[-suffix].json` (authored upstream by regression stage)
- Synced page spec remains the upstream source for field/status wording; closeout does not rewrite it wholesale

### Stop conditions (all must be true)

| ID | Check | Quantifier |
|---|---|---|
| SC-CLOSE-1 | phase_gate | `phase-gate.mjs --target-phase closeout` exits 0 |
| SC-CLOSE-2 | validate_manifest | `split-manifest.json` has zero schema/provenance errors after closeout sync |
| SC-CLOSE-3 | regression_manifest_valid | `regression-checklist.json` exists and `validate-manifest.mjs` exits 0 |
| SC-CLOSE-4 | regression_refs_only | `split-manifest.regressionChecklist` contains only id/ref strings, not full Gate bodies |
| SC-CLOSE-5 | cross_artifact_alignment | every `split-manifest.regressionChecklist` entry maps to a Gate/ref in `regression-checklist.json`, and the affected page spec has already been synced upstream |
...
| `prototype-split-closeout` | closeout | closeout gate | split-manifest + regression-checklist | — | — |
