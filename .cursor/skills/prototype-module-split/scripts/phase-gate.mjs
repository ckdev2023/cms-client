...
function checkReviewGate(moduleDirAbs, suffix) {
  ...
  const targetShorthands = new Set((bm.targetFiles || []).map((t) => t.shorthand));
  for (const entry of bm.loadOrder || []) {
  ...
  const am = tryLoadJson(amPath);
  if (am && bm.functions) {
    const assignedNames = new Set(bm.functions.map((f) => f.name));
    const bmUnknownIds = new Set((bm.unknowns || []).map((u) => u.id));
    const bmWaiverIds = new Set((bm.waivers || []).map((w) => w.id));
    for (const hf of am.helperFunctions || []) {
      if (!assignedNames.has(hf.name) && !bmUnknownIds.has(hf.name) && !bmWaiverIds.has(hf.name)) {
        warnings.push(
          `audit helperFunction "${hf.name}" has no boundary-map assignment, unknown, or waiver`
        );
      }
    }
  }

  if (bm.unknowns && bm.unknowns.length > 0) {
    const blocking = bm.unknowns.filter((u) => u.impact === "blocking");
    if (blocking.length > 0) {
      warnings.push(
        `boundary-map has ${blocking.length} blocking unknown(s) to address during review: ` +
  ...
  if (bm) {
    ...
    if (!approval || approval.approved !== true) {
      ...
      if (!approval.approvedBy) {
        errors.push(
          "boundary-map.phase.reviewApproval.approvedBy must identify the reviewer"
        );
      }
      if (approval.disputedItems && approval.disputedItems.length > 0) {
        warnings.push(
          `review has ${approval.disputedItems.length} disputed item(s): ` +
          approval.disputedItems.join(", ")
        );
      }
    }
    ...
    if (bm.unknowns && bm.unknowns.length > 0) {
      ...
      if (blocking.length > 0) {
        errors.push(
          `boundary-map has ${blocking.length} blocking unknown(s) that must be resolved or waived before execute: ` +
          blocking.map((u) => u.id).join(", ")
        );
      }
    }
