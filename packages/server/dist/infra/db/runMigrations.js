...
function readSql(migrationsDir, fileName) {
...
function buildMigrationIndex(migrationsDir) {
...
async function rollbackMigration(pool, id, sql) {
  await pool.query("begin");
  try {
    await pool.query(sql);
...
async function rollback(migrationsDir, argv) {
  const env = loadEnv();
  const pool = createPgPool(env.dbUrl);
  try {
    const index = buildMigrationIndex(migrationsDir);
