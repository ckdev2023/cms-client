...
import { createTenantDb } from "./modules/core/tenancy/tenantDb";
...
async function bootstrapWorker() {
  ...
  try {
    await Promise.all([
      pgWorker(),
      queue.runWorker("reminder_jobs", (job) =>
        handleReminderJob(pool, queue, job),
      ),
      queue.runWorker("notification_jobs", (job) =>
        handleNotificationJob(pool, notificationAdapter, job),
      ),
      queue.runWorker("translation_jobs", (job) =>
        handleTranslationJob(pool, translationAdapter, job),
      ),
      queue.runWorker("export_jobs", (job) =>
 ...
 * @returns 组织 ID 列表
 */
async function listActiveOrganizationIds(pool) {
 ...
 * @returns job 或 null
 */
async function claimNextJob(tenantDb, workerId, lockTtlSeconds) {
 ...
 * @param jobId job ID
 */
async function markJobSucceeded(tenantDb, jobId) {
 ...
 * @param e 异常
 */
async function markJobFailedOrRetry(tenantDb, job, e) {
 ...
 * @param job job 记录
 */
async function executeJob(pool, job) {
 ...
 * @param job job 记录
 */
async function handleTimelineWrite(pool, job) {
 ...
 */
function parseTimelineWritePayload(raw) {
  const p = normalizeObject(raw);
 ...
 * @returns 秒数
 */
function readWorkerLockTtlSecondsFromEnv() {
 ...
 * @param workerId worker 标识
 */
async function releaseWorkerLocks(pool, workerId) {
