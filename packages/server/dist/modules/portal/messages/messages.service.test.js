...
function makeMockRedis() {
  ...
  const client = {
    isOpen: true,
    rPush: (key, value) => {
      enqueueCalls.push({ queueName: key, job: JSON.parse(value) });
...
function createService(poolFn) {
  const pool = makePool(poolFn);
  const { client, enqueueCalls } = makeMockRedis();
  const svc = new MessagesService(pool, client);
...
//# sourceMappingURL=messages.service.test.js.map
