...
void test("AppUserAuthGuard sets appUserContext for valid token", () => {
  ...
  assert.equal(ctx._req.appUserContext.appUserId, "au-1");
});
// ── Missing token throws ──
void test("AppUserAuthGuard throws for missing authorization", () => {
  const guard = new AppUserAuthGuard();
  const ctx = makeContext({});
  assert.throws(() => guard.canActivate(ctx), /Missing authorization/);
});
...
void test("AppUserAuthGuard throws for invalid token", () => {
  ...
  assert.throws(() => guard.canActivate(ctx), /Invalid or expired token/);
});
// ── Wrong secret throws ──
...
//# sourceMappingURL=appUserAuth.guard.test.js.map
