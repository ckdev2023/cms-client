 ...
 */
let AppUserAuthService = class AppUserAuthService {
  pool;
  redis;
  /**
   * 创建认证服务实例。
  ...
  async findAppUserByIdentifier(type, value) {
    // P2-1: SQL 列名白名单防注入
 ...
 */
export function signAppUserJwt(appUserId, secret) {
 ...
 */
export function verifyAppUserJwt(token, secret) {
...
function verifySignature(hB64, pB64, sigB64, secret) {
...
function parsePayload(pB64) {
  try {
    return JSON.parse(Buffer.from(pB64, "base64url").toString());
  } catch {
    return null;
  }
}
...
//# sourceMappingURL=appUserAuth.service.js.map
