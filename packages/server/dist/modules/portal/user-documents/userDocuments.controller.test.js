...
void test("UserDocumentsController list parses query and validates UUID filters", async () => {
  ...
  await assert.rejects(
    () => controller.list(req, { leadId: "bad" }),
    /Invalid leadId/,
  );
  await assert.rejects(
    () => controller.list(req, { caseId: "bad" }),
    /Invalid caseId/,
  );
...
void test("UserDocumentsController get validates id and ownership", async () => {
  ...
  const controller = new UserDocumentsController(service);
  await assert.rejects(() => controller.get({}, DOC_ID), UnauthorizedException);
  await assert.rejects(() => controller.get(req, "bad"), /Invalid id/);
  await assert.rejects(
    () => controller.get(req, CASE_ID),
    /Document not found/,
  );
  await assert.rejects(
  ...
  const res = await controller.get(req, DOC_ID);
  assert.equal(res.id, DOC_ID);
});
void test("UserDocumentsController downloadUrl validates id and ownership", async () => {
  ...
  const controller = new UserDocumentsController(service);
  await assert.rejects(
    () => controller.downloadUrl({}, DOC_ID),
    UnauthorizedException,
  );
  await assert.rejects(() => controller.downloadUrl(req, "bad"), /Invalid id/);
  await assert.rejects(
    () => controller.downloadUrl(req, CASE_ID),
    /Document not found/,
  );
  await assert.rejects(
    () => controller.downloadUrl(req, ORG_ID),
    /Cannot access other user's document/,
  );
  const res = await controller.downloadUrl(req, DOC_ID);
...
void test("UserDocumentsController remove validates id and ownership", async () => {
  ...
  await assert.rejects(
    () => controller.remove(req, CASE_ID),
    /Document not found/,
  );
  await assert.rejects(
    () => controller.remove(req, ORG_ID),
    /Cannot delete other user's document/,
  );
  const res = await controller.remove(req, DOC_ID);
  assert.equal(res.ok, true);
  assert.equal(removedId, DOC_ID);
});
//# sourceMappingURL=userDocuments.controller.test.js.map
