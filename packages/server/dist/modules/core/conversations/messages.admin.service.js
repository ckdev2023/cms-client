 ...
 * Admin 側メッセージ管理サービス。
 */
let MessagesAdminService = class MessagesAdminService {
  pool;
  redisClient;
   ...
   */
  constructor(pool, redisClient, conversationsAdminService) {
    this.pool = pool;
    this.redisClient = redisClient;
    this.conversationsAdminService = conversationsAdminService;
   ...
   * @returns メッセージ一覧と総件数
   */
  async list(ctx, conversationId, input) {
    ...
    const dataRes = await tenantDb.query(
   ...
   * @returns 作成されたメッセージ
   */
  async send(ctx, conversationId, input) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
   ...
   * @returns 更新後のメッセージ
   */
  async retryTranslation(ctx, conversationId, messageId) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
    const msgRes = await tenantDb.query(
  ...
  async assertConversationExists(tenantDb, conversationId) {
    const convCheck = await tenantDb.query(
  ...
  async updateConversationOnSend(tenantDb, conversationId) {
    await tenantDb.query(
      `update conversations set last_message_at = now(), unread_count_user = unread_count_user + 1, updated_at = now() where id = $1`,
      [conversationId],
    );
  }
  async enqueueTranslation(orgId, message, originalLanguage, prefix = "tj") {
...
MessagesAdminService = __decorate(
  [
    Injectable(),
    __param(0, Inject(Pool)),
    __param(1, Inject(REDIS_CLIENT)),
    __param(2, Inject(ConversationsAdminService)),
    __metadata("design:paramtypes", [Pool, Object, ConversationsAdminService]),
  ],
  MessagesAdminService,
);
export { MessagesAdminService };
//# sourceMappingURL=messages.admin.service.js.map
