...
function makeResidencePeriodRow(overrides = {}) {
  return {
    id: PERIOD_ID,
    org_id: ORG_ID,
    case_id: CASE_ID,
...
async function createReminderScenario(options) {
...
void test("create uses BMV template blueprint when case_templates row exists", async () => {
  ...
  const dedupeKeys = insertedReminders.map((p) => p[8]);
...
void test("payload_snapshot includes residencePeriodId, validUntil, daysBefore, statusOfResidence", async () => {
  ...
  await createReminderScenario({
    onInsertReminder: (params) => {
      const raw = params[9];
      insertedPayloads.push(JSON.parse(raw));
    },
  });
...
//# sourceMappingURL=residencePeriods.reminder-blueprint.test.js.map
