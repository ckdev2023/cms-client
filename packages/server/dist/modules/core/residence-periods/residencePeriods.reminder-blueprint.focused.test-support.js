 ...
 */
export function makeResidencePeriodRow(overrides = {}) {
  return {
    id: PERIOD_ID,
    org_id: ORG_ID,
    case_id: CASE_ID,
 ...
 */
export function createService(queryFn) {
  const timeline = makeTimeline();
  const svc = new ResidencePeriodsService(makePool(queryFn), timeline.service);
  return { svc, timeline };
}
...
function routeCaseAndCustomerLookups(sql) {
...
function routeResidencePeriodWrite(sql, rowCount, row) {
  if (
...
function routeTemplateBlueprintLookup(sql, templateBlueprint, emptyRowCount) {
...
function routeReminderLifecycle(sql, params, opts) {
  if (
...
function routeSavepoint(sql, onSavepoint) {
 ...
 * @returns 供 `createService` 使用的查询函数。
 */
export function reminderFailureQueryFn(opts = {}) {
 ...
 */
export function makeCaseEntity(overrides = {}) {
  return {
    id: CASE_ID,
    orgId: ORG_ID,
    customerId: CUSTOMER_ID,
...
//# sourceMappingURL=residencePeriods.reminder-blueprint.focused.test-support.js.map
