 ...
 * @param caseId 案件 ID
 */
export async function syncBillingCacheForCase(tx, caseId) {
  const billingResult = await tx.query(
    `select amount_due, status, milestone_name, gate_effect_mode
     from billing_records where case_id = $1`,
    [caseId],
  );
  const validPaymentResult = await tx.query(
 ...
 */
export async function checkFinalPaymentGuard(tx, caseId) {
  ...
  const payResult = await tx.query(
  ...
  return {
    ...
    gateEffectMode: gateMode,
  };
}
function isDepositMilestone(name) {
...
function isFinalPaymentMilestone(name) {
  if (!name) return false;
  const lower = name.toLowerCase();
  return (
...
//# sourceMappingURL=billingGuards.js.map
