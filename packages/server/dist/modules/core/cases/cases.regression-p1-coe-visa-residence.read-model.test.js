...
import { mapCaseRow } from "./cases.service";
...
void describe("§23 mapCaseRow: P1 COE/visa/residence field mapping", () => {
  ...
  void test("maps result_outcome string", () => {
    const c = mapCaseRow(makeCaseRow({ result_outcome: "rejected" }));
    assert.equal(c.resultOutcome, "rejected");
  });
  void test("maps result_outcome null", () => {
    const c = mapCaseRow(makeCaseRow({ result_outcome: null }));
...
//# sourceMappingURL=cases.regression-p1-coe-visa-residence.read-model.test.js.map
