...
// ── helpers ──
function makeSettledGuard() {
  return { settled: true };
}
function makeUnsettledGuard(mode, unpaid = 200000) {
...
void describe("final_payment constants freeze", () => {
  ...
  void test("milestone and deposit keywords are disjoint", () => {
    const ms = new Set(FINAL_PAYMENT_MILESTONE_KEYWORDS);
...
void describe("FinalPaymentSourceOfTruth: billingGuards alignment", () => {
  void test("keywords align with isFinalPaymentMilestone heuristic", () => {
    for (const kw of ["尾款", "final", "結果"]) {
      assert.ok(FINAL_PAYMENT_MILESTONE_KEYWORDS.includes(kw));
    }
  });
  void test("type shape structurally complete", () => {
    const sot = FINAL_PAYMENT_SOURCE_OF_TRUTH;
    for (const k of [
      "primaryTable",
...
//# sourceMappingURL=cases.types-final-payment.test.js.map
