...
function svc(pool) {
  return new CasesService(pool, makeTemplates().service);
}
function ctx() {
...
void test("§8 create: forwards groupId + crossGroupReason", async () => {
  ...
  await ctrl.create(reqCtx("staff"), {
    customerId: "c1",
    caseTypeCode: "visa",
    ownerUserId: "u1",
    groupId: "cross-group",
...
void test("§8 update: forwards groupId + groupTransferReason", async () => {
  ...
  const ctrl = new CasesController(service, stubPerms());
  await ctrl.update(reqCtx("staff"), "case-1", {
    groupId: "new-group",
...
//# sourceMappingURL=cases.regression-service.test.js.map
