 ...
 * @see BUG-063
 */
export const DEFAULT_CASE_TRANSITIONS = {
  S1: ["S2"],
  S2: ["S3"],
  S3: ["S2", "S4"],
  S4: ["S3", "S5"],
...
function resolvePostApprovalStage(columnValue, metadata) {
...
function mapCaseExtendedFields(row) {
 ...
 *
 */
export function mapCaseListSummaryRow(row) {
  return {
    ...mapCaseRow(row),
 ...
 *
 */
export function mapDetailCountsRow(row) {
 ...
 *
 */
export function mapLatestSubmissionRow(row) {
  if (!row) return null;
  return {
    id: row.id,
    submissionNo:
 ...
 *
 */
export function mapLatestReviewRow(row) {
  if (!row) return null;
  return {
    id: row.id,
 ...
 *
 */
export function mapDocProgressByProviderRows(rows) {
  return rows.map((row) => ({
 ...
 *
 */
export function mapLatestValidationRow(row) {
  if (!row) return null;
  return {
    id: row.id,
...
function appendVisibilityConditionPrefixed(where, params, v, prefix) {
...
function pickDefined(v, fallback) {
  // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
  return v !== undefined ? v : fallback;
}
const DEFAULT_CASE_PREFIX = "CASE";
function formatCaseYearMonth(date) {
  return `${String(date.getFullYear())}${String(date.getMonth() + 1).padStart(2, "0")}`;
}
function formatCaseNo(prefix, date, seq) {
  return `${prefix}-${formatCaseYearMonth(date)}-${String(seq).padStart(4, "0")}`;
}
function resolveCasePrefix(settings) {
  if (!settings || typeof settings !== "object" || Array.isArray(settings)) {
    return DEFAULT_CASE_PREFIX;
  }
  const value = settings.case_prefix;
  return typeof value === "string" && value.trim().length > 0
    ? value.trim()
    : DEFAULT_CASE_PREFIX;
}
function isCaseNoConflict(error) {
...
function resolveCaseUpdateFields(input, current) {
  return {
 ...
 * 案件进入 S9 后除日志外全字段只读。
 */
function assertNotArchived(current) {
...
function resolveRequestedTransitionStage(input) {
  if (
...
function resolveInitialBusinessPhase(stage) {
  if (stage in STAGE_TO_PHASE_DEFAULT) {
    return STAGE_TO_PHASE_DEFAULT[stage];
  }
  return "CONSULTING";
}
/**
 ...
 * 其余阶段走默认映射。
 */
function resolveTransitionBusinessPhase(toStage, closeReason) {
...
function buildInsertCaseParams(orgId, input) {
...
async function queryDetailCaseRow(tenantDb, id) {
...
async function queryDetailCounts(tenantDb, id) {
  const result = await tenantDb.query(
    `
      select
...
async function queryLatestValidation(tenantDb, id) {
  const result = await tenantDb.query(
    `
...
async function queryLatestSubmission(tenantDb, id) {
  const result = await tenantDb.query(
    `
...
async function queryLatestReview(tenantDb, id) {
  const result = await tenantDb.query(
    `
...
async function queryDocProgressByProvider(tenantDb, id) {
  const result = await tenantDb.query(
    `
      select
...
async function queryCurrentResidencePeriod(tenantDb, caseId) {
  const result = await tenantDb.query(
    `
  ...
  if (!row) return null;
  const period = mapResidencePeriodRow(row);
  return toResidencePeriodSummary(period);
}
function deriveBillingSummary(caseEntity) {
  return {
...
function deriveDeepLink(caseEntity, caseRow) {
  return {
...
function validateWorkflowStepTransitionTarget(current, toStepCode) {
...
const OVERSEAS_STEP_CODE_SET = new Set(Object.values(OVERSEAS_STEP_CODES));
/**
 *
 */
export function isOverseasStepCode(code) {
 ...
 */
export function resolveOverseasStepEffects(current, toStepCode) {
  return {
    stampCoeSent:
 ...
 * @see BUG-098
 */
export function resolvePhaseStampEffects(current, toPhase) {
  return {
 ...
 * @see BUG-099
 */
export function shouldIncrementSupplementCount(fromPhase, toPhase) {
  return fromPhase === "UNDER_REVIEW" && toPhase === "NEED_SUPPLEMENT";
}
/**
 * 终态 phase → S9 映射。非终态返回 null（stage 不动）。
 */
export function mapPhaseToTerminalStage(phase) {
...
function settledValueOrUndefined(result) {
...
function logSettledErrors(results, caseId) {
...
let CasesService = class CasesService {
  ...
  async create(ctx, input) {
    try {
      ...
      CasesService_1.wrapCreateError(error, input);
    }
  }
  async runCreateTransaction(tx, ctx, input, checklistItems) {
  ...
  static PG_CLIENT_ERROR_REASONS = {
  ...
  static wrapCreateError(error, input) {
    if (error instanceof HttpException) throw error;
    const pgCode = error.code;
    const reason = pgCode
      ? CasesService_1.PG_CLIENT_ERROR_REASONS[pgCode]
      : undefined;
    if (pgCode && reason) {
  ...
  async assertCreateRefs(tx, input) {
  ...
  async assertBmvCaseCreationGate(tx, input) {
  ...
  async resolveCreateGroup(tx, ctx, input) {
   ...
   */
  async resolveExplicitGroupId(tx, orgId, raw) {
   ...
   */
  async resolveOwnerUserId(tx, raw, fallbackUserId) {
   ...
   */
  async assertCanEditCase(ctx, caseId) {
   ...
   */
  async getBillingTabAggregate(ctx, caseId) {
  ...
  async aggregateCaseBillingSummaryFull(tenantDb, caseId) {
    const result = await tenantDb.query(
      `select
        c.quote_price,
   ...
   */
  async getDetailAggregate(ctx, id) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
   ...
   * @returns 更新后的 Case
   */
  async acknowledgeBillingRisk(ctx, id, input) {
    const current = await this.get(ctx, id);
    if (!current) throw new NotFoundException("Case not found or deleted");
    assertNotArchived(current);
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
   ...
   * @returns 更新后的 Case
   */
  async updatePostApprovalStage(ctx, id, input) {
  ...
  async assertPostApprovalBillingGate(tx, current, nextStage) {
    if (nextStage !== "coe_sent") return;
    await this.runCoeSendBillingGate(tx, current);
  }
  async runCoeSendBillingGate(tx, current) {
  ...
  async hasFinalPaymentBillingRecords(tx, caseId) {
    const result = await tx.query(
  ...
  async transitionWorkflowStep(ctx, id, input) {
    const current = await this.get(ctx, id);
    if (!current) throw new NotFoundException("Case not found or deleted");
    assertNotArchived(current);
   ...
   *   - 必须提供 closeReason
   */
  async transitionPhase(ctx, id, input) {
    const current = await this.get(ctx, id);
    if (!current) throw new NotFoundException("Case not found or deleted");
    assertNotArchived(current);
  ...
  assertCloseReasonForFailedPhase(toPhase, closeReason) {
  ...
  buildPhaseTransitionEffects(current, fromPhase, toPhase, input) {
    const closeReason =
  ...
  async executePhaseTransitionUpdate(tx, id, fromPhase, toPhase, effects) {
    const result = await tx.query(
      `update cases
   ...
   */
  async assertCoeSendBillingGate(tx, current, toPhase) {
   ...
   */
  async assertWaitingPaymentBillingGate(tx, current, toPhase) {
  ...
  async assertClosedSuccessGate(ctx, caseId) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
   ...
   */
  async writeOverseasStepTimeline(tx, ctx, updated, toStepCode) {
  ...
  async assertWorkflowStepBillingGate(tx, current, toStepCode) {
   ...
   */
  async resolveChecklistItems(ctx, caseTypeCode) {
    ...
    const rawItems = Array.isArray(resolved.config.items)
      ? resolved.config.items
      : Array.isArray(resolved.config.requirementBlueprint)
        ? resolved.config.requirementBlueprint
        : [];
  ...
  async validateTransition(ctx, c, from, to, closeReason) {
    ...
    if (resolved.mode === "template" && resolved.used) {
      const ts = Array.isArray(resolved.config.allowedTransitions)
        ? resolved.config.allowedTransitions
        : [];
    ...
    const allowed = DEFAULT_CASE_TRANSITIONS[from];
  ...
  async validateTransitionGate(ctx, c, from, to, closeReason) {
    if (from === "S3" && to === "S4") {
  ...
  async validateReadyForDocumentPreparation(ctx, c) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
    const primaryPartyResult = await tenantDb.query(
      `
        select id
        from case_parties
        where org_id = $1
   ...
   */
  async validateReadyForInternalReview(ctx, c) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
    const incompleteRequiredItemResult = await tenantDb.query(
      `
        select id
        from document_items
        where org_id = $1
  ...
  async getLatestValidationRun(tenantDb, orgId, caseId) {
  ...
  async getLatestReviewRecord(tenantDb, orgId, caseId, validationRunId) {
    const reviewResult = await tenantDb.query(
      `
        select id, decision
        from review_records
        where org_id = $1
   ...
   */
  async validateReadyForSubmission(ctx, c) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
   ...
   */
  async validateSuccessCloseout(ctx, c, closeReason) {
   ...
   */
  async assertLatestValidationRunPassed(tenantDb, ctx, c, fromLabel, toLabel) {
   ...
   */
  async isValidationRunStale(tenantDb, orgId, caseId, validationExecutedAt) {
    const result = await tenantDb.query(
      `select (
         exists (
  ...
  async insertCaseWithAutoNumber(tx, ctx, input) {
  ...
  async executeUpdateCase(tx, id, f) {
    const result = await tx.query(
      `update cases
  ...
  async insertDocumentItems(tx, orgId, caseId, items) {
  ...
  async insertInitialTasks(tx, ctx, created) {
    const seeds = [
      {
        title: "邀请客户上传基础资料",
   ...
   */
  async resolveCustomerGroupId(tx, customerId) {
    const result = await tx.query(
   ...
   *
   */
  async incrementSupplementCount(ctx, caseId) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
    return tenantDb.transaction(async (tx) =>
  ...
  static ALLOWED_ASSERT_TABLES = new Set(["customers", "users", "companies"]);
  /** 断言记录属于当前 org（RLS 过滤 + 表名白名单防注入）。
   * @param table
   * @param id
   * @param tx 事务连接 @param table 表名 @param id 记录 ID */
  async assertBelongsToOrg(tx, table, id) {
    if (!CasesService_1.ALLOWED_ASSERT_TABLES.has(table)) {
      throw new Error(`assertBelongsToOrg: disallowed table "${table}"`);
    }
    const r = await tx.query(`select id from ${table} where id = $1 limit 1`, [
      id,
    ]);
    if (r.rows.length === 0)
      throw new BadRequestException(
...
CasesService = CasesService_1 = __decorate(
  [
    ...
    __param(1, Inject(TEMPLATES_RESOLVER)),
    __param(2, Optional()),
    __param(2, Inject(PermissionsService)),
    __param(3, Optional()),
    __param(3, Inject(BillingPlansService)),
    __param(4, Optional()),
    __param(4, Inject(PaymentRecordsService)),
...
async function writeTimelineInTx(tx, ctx, input) {
  await tx.query(
...
async function insertInitialBillingPlanFromQuote(tx, ctx, caseId, quotePrice) {
  if (
    quotePrice === null ||
