...
void test("list with view=summary calls listSummary", async () => {
  ...
  const result = await controller.list(viewerCtxReq, {
    scope: "mine",
    view: "summary",
  });
...
void test("getAggregate returns aggregate DTO", async () => {
  ...
  const controller = new CasesController(service, makePermissions());
  const result = await controller.getAggregate(viewerCtxReq, "case-1");
  assert.equal(result.deepLink.customerName, "Alice");
  assert.equal(result.counts.documentItemsTotal, 10);
  assert.equal(result.counts.generatedDocuments, 2);
  assert.equal(result.documentProgressByProvider.length, 1);
});
void test("getAggregate throws when case not found", async () => {
  ...
  await assert.rejects(
    () => controller.getAggregate(viewerCtxReq, "missing"),
    BadRequestException,
  );
});
void test("getAggregate throws ForbiddenException when canViewCase denies", async () => {
  ...
  await assert.rejects(
    () => controller.getAggregate(viewerCtxReq, "case-1"),
    ForbiddenException,
  );
});
void test("getAggregate: missing requestContext throws UnauthorizedException", async () => {
  ...
  const controller = new CasesController(service, makePermissions());
  await assert.rejects(
    () => controller.getAggregate({}, "case-1"),
    UnauthorizedException,
  );
});
//# sourceMappingURL=cases.controller.read-model.test.js.map
