...
export const ok = (rows = [], rowCount = rows.length) =>
...
function applyStageStatusFallback(row, overrides) {
 ...
 */
export function makeTemplates(r) {
  return {
    service: {
      resolve: (_ctx, input) =>
        Promise.resolve(
 ...
 */
export function svc(pool, tpl) {
  return new CasesService(pool, (tpl ?? makeTemplates()).service);
}
/**
 * 生成默认测试上下文。
 ...
 */
export function makeCountsRow(overrides = {}) {
  return {
...
//# sourceMappingURL=cases.regression-p1-questionnaire-supplement.test-support.js.map
