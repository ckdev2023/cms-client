 ...
 */
export function makeSubmissionPackageRow(overrides = {}) {
  return {
    id: PACKAGE_ID,
    org_id: ORG_ID,
    case_id: CASE_ID,
 ...
 */
export function makeSubmissionPackageItemRow(overrides = {}) {
  return {
    id: ITEM_ID,
...
export function createService(
  ...
  const svc = new SubmissionPackagesService(
    ...
    timeline.service,
    cases.service,
    makeTemplatesResolver(reviewRequired),
  );
  return { svc, timeline, cases };
}
function handleValidationRunQuery(sql) {
...
function handleCaseLookupQuery(sql) {
  if (!sql.includes("select id from cases")) {
    return null;
  }
  return { rows: [{ id: CASE_ID }], rowCount: 1 };
}
function handleRelatedSubmissionQuery(sql) {
  if (
...
function handleSubmissionNumberQuery(sql) {
  if (!sql.includes("coalesce(max(submission_no), 0) + 1")) {
    return null;
  }
  return { rows: [{ next_submission_no: "2" }], rowCount: 1 };
}
function handleInsertSubmissionPackageQuery(sql, params, overrides) {
...
function handleDocumentItemsQuery(sql) {
...
function handleGeneratedDocumentsQuery(sql) {
 ...
 */
export function standardQueryFn(overrides = {}) {
  return (sql, params) =>
    Promise.resolve(resolveStandardQuery(sql, params, overrides));
}
//# sourceMappingURL=submissionPackages.supplement-count.focused.test-support.js.map
