...
function ownerMetaLabel(ownerName) {
  return ownerName ? `负责人：${ownerName}` : undefined;
}
function dueMetaLabel(dueAt) {
 ...
 * @returns 供前端渲染的待办工作项。
 */
export function mapTodoItem(row) {
 ...
 */
export function mapDeadlineItem(row) {
  const daysLeft = Number(row.days_left);
  const tone = daysLeft <= 3 ? "danger" : daysLeft <= 7 ? "warn" : "muted";
 ...
 */
export function mapSubmissionItem(row) {
...
function describeRisk(row, unpaidAmount) {
 ...
 * @returns 供前端渲染的风险工作项。
 */
export function mapRiskItem(row) {
...
//# sourceMappingURL=dashboard.workItem.js.map
