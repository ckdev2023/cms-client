...
export { mapDocumentItemRow } from "./documentItems.shared";
export const ALLOWED_TRANSITIONS = DOCUMENT_ITEM_ALLOWED_TRANSITIONS;
/** DocumentItem 服务，提供 CRUD、状态变更、催办、survey_data 与软删除能力。 */
let DocumentItemsService = class DocumentItemsService {
  pool;
  timelineService;
   ...
   * @returns 完成率汇总
   */
  async getCompletionRate(ctx, caseId) {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
    const result = await tenantDb.query(
   ...
   * @returns 更新后的资料项
   */
  async updateSurveyData(ctx, id, input) {
    const current = await this.get(ctx, id);
    if (!current) throw new NotFoundException("Document item not found");
   ...
   * @returns 更新后的资料项
   */
  async waive(ctx, id, input) {
    const current = await this.get(ctx, id);
    if (!current) throw new NotFoundException("Document item not found");
    if (!WAIVE_ALLOWED_FROM_STATUSES.includes(current.status)) {
      throw new BadRequestException(
...
export { DocumentItemsService };
//# sourceMappingURL=documentItems.service.js.map
