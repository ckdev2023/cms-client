...
function createService(pool, timeline) {
  return new DocumentItemsService(pool, timeline.service);
}
// ── create ──
void test("DocumentItemsService.create inserts row and writes timeline", async () => {
  ...
  assert.deepEqual(timeline.writes[0].action, "document_item.created");
});
// ── create failure ──
...
void test("DocumentItemsService.update updates and writes timeline", async () => {
  ...
  assert.equal(timeline.writes.length, 1);
  assert.deepEqual(timeline.writes[0].action, "document_item.updated");
});
// ── update not found ──
...
function transitionPool(toStatus, fromStatus) {
...
const ALL_VALID_PAIRS = Object.entries(ALLOWED_TRANSITIONS).flatMap(
...
for (const [from, to] of ALL_VALID_PAIRS) {
  void test(`transition: allows ${from} → ${to}`, async () => {
    ...
    assert.equal(timeline.writes.length, 1);
    assert.deepEqual(timeline.writes[0].action, "document_item.transitioned");
  });
}
...
for (const validStatus of ["waiting_upload", "revision_required"]) {
  void test(`followUp: allowed in ${validStatus}`, async () => {
    ...
    assert.equal(timeline.writes.length, 1);
    assert.deepEqual(timeline.writes[0].action, "document_item_follow_up");
  });
}
// ── followUp not found ──
...
void test("DocumentItemsService.softDelete marks as deleted and writes timeline", async () => {
  ...
  assert.equal(timeline.writes.length, 1);
  assert.deepEqual(timeline.writes[0].action, "document_item.deleted");
});
...
for (const fromStatus of [
  ...
  void test(`waive: allows from ${fromStatus}`, async () => {
    ...
    assert.equal(timeline.writes.length, 1);
    assert.deepEqual(timeline.writes[0].action, "document_item.waived");
  });
}
void test("waive: writes timeline with reasonCode and note", async () => {
  ...
  assert.equal(timeline.writes.length, 1);
  const payload = timeline.writes[0].payload;
...
//# sourceMappingURL=documentItems.service.test.js.map
