...
import { handleNotificationJob } from "./notificationJobHandler";
/* ------------------------------------------------------------------ */
/*  常量 & 工具                                                        */
/* ------------------------------------------------------------------ */
...
function makePool(queryFn) {
  const calls = [];
  const defaultQueryFn = (sql, params) => {
...
void test("sends notification via adapter and writes timeline when entityType+entityId present", async () => {
  ...
  const payloadJson = JSON.parse(p[5]);
...
void test("timeline payload does NOT include body content", async () => {
  ...
  const payloadJson = JSON.parse(timelineCall.params?.[5]);
  assert.equal(
    "body" in payloadJson,
    false,
...
//# sourceMappingURL=notificationJobHandler.test.js.map
