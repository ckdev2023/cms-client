 ...
 * @returns 存储 key
 */
function buildFileKey(orgId, exportType, ext) {
 ...
 * @param job 队列任务
 */
export async function handleExportJob(pool, storageAdapter, queue, job) {
 ...
 * @returns 数据行
 */
async function queryExportData(tenantDb, exportType) {
  const sql = QUERY_MAP[exportType];
  const result = await tenantDb.query(sql, [MAX_EXPORT_ROWS]);
  return result.rows;
}
/**
 * 构建导出文件。
 ...
 * @returns 文件信息
 */
function buildExportFile(rows, format) {
  if (format === "csv") {
    return {
...
//# sourceMappingURL=exportJobHandler.js.map
