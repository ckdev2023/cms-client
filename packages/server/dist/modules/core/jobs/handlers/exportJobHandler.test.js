...
import { handleExportJob } from "./exportJobHandler";
/* ------------------------------------------------------------------ */
/*  常量                                                               */
/* ------------------------------------------------------------------ */
...
function makeDefaultPool(rows = [makeCaseRow()]) {
  const calls = [];
...
void test("exports cases as CSV: queries data, uploads file, enqueues notification, writes timeline", async () => {
  ...
  await handleExportJob(pool, adapter, queue, makeJob());
  // 验证查询了 cases 表
  ...
  assert.ok(p.body.includes("cases"), "body should mention export type");
...
void test("propagates storage upload error", async () => {
  ...
  await assert.rejects(
    () => handleExportJob(pool, adapter, queue, makeJob()),
    /Storage unavailable/,
  );
});
/* 9. queue enqueue 失败 */
...
void test("excel format falls back to CSV (TODO placeholder)", async () => {
  ...
  const { adapter, uploadCalls } = makeMockStorage();
  const { queue } = makeMockQueue();
  await handleExportJob(pool, adapter, queue, makeJob({ format: "excel" }));
  assert.equal(uploadCalls.length, 1);
  // 暂时回退为 CSV
  assert.equal(uploadCalls[0]?.contentType, "text/csv");
});
...
void test("query limits all export types to 10000 rows", async () => {
  ...
  const { adapter } = makeMockStorage();
  const { queue } = makeMockQueue();
  for (const et of ["cases", "customers", "document_items"]) {
    calls.length = 0;
    await handleExportJob(pool, adapter, queue, makeJob({ exportType: et }));
...
void test("timeline payload includes exportType, format, fileKey, rowCount", async () => {
  ...
  assert.ok(tl);
  const parsed = JSON.parse(tl.params?.[5]);
...
//# sourceMappingURL=exportJobHandler.test.js.map
