...
let CommunicationLogsService = class CommunicationLogsService {
  ...
  async list(ctx, input = {}) {
    ...
    return tenantRepo.query(async ({ db }) => {
      ...
      const result = await db
        ...
        .offset(offset);
      const creatorIds = [
        ...new Set(result.map((r) => r.createdBy).filter((id) => id !== null)),
      ];
   ...
   */
  buildRelationWhere(input, extra = []) {
  ...
  async writeCreateTimelines(ctx, created) {
  ...
  async writeUpdateTimelines(ctx, current, updated) {
    await this.timelineService.write(ctx, {
      entityType: "communication_log",
      entityId: updated.id,
  ...
  async assertRefs(assertBelongsToOrg, caseId, customerId, companyId) {
  ...
  async writeCaseTimelinesForUpdate(ctx, current, updated) {
...
CommunicationLogsService = __decorate(
  [
    Injectable(),
...
export { CommunicationLogsService };
//# sourceMappingURL=communicationLogs.service.js.map
