...
function service(pool, tl) {
  return new CommunicationLogsService(pool, tl.service);
}
...
void test("CommunicationLogsService.update updates row and writes case timeline", async () => {
  ...
  assert.equal(tl.writes.length, 2);
  assert.equal(tl.writes[0].entityType, "communication_log");
  assert.equal(tl.writes[1].entityType, "case");
});
...
//# sourceMappingURL=communicationLogs.service.test.js.map
