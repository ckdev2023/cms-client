...
function makeCustomerStub(items) {
  return { list: () => Promise.resolve({ items, total: items.length }) };
}
function makeCaseStub(items) {
  return {
    listSummary: () =>
      Promise.resolve({
        items,
...
function makeLeadStub(items) {
  return { list: () => Promise.resolve({ items, total: items.length }) };
}
function makeTaskStub(items) {
  return { list: () => Promise.resolve({ items, total: items.length }) };
}
function makeDocStub(items) {
  return { list: () => Promise.resolve({ items, total: items.length }) };
}
function makeConvStub(items) {
  return { list: () => Promise.resolve({ items, total: items.length }) };
}
...
void test("partial failure: one service throws, others still return", async () => {
  const service = createService({
    customers: { list: () => Promise.reject(new Error("DB fail")) },
    leads: makeLeadStub([
      {
        id: "l1",
        name: "Test Lead",
...
void test("query is trimmed before search", async () => {
  ...
  const service = createService({ customers: customerStub });
  const result = await service.search(makeCtx(), "  hello  ");
