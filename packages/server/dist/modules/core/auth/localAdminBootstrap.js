...
const DEFAULT_LOCAL_ROLE = "owner";
 ...
 * @returns 初始化输入
 */
export function readLocalAdminBootstrapInput(env = process.env) {
 ...
 * @returns 初始化结果
 */
export async function bootstrapLocalAdmin(pool, input) {
...
async function upsertOrganization(client, input) {
...
async function upsertAdminUser(client, input, orgId) {
...
function buildDefaultLocalDbUrl() {
  const url = new URL("postgres://localhost:5433/cms");
  url.username = "cms";
  url.password = "cms";
  return url.toString();
}
function readOptionalEnv(key, env, fallback) {
...
//# sourceMappingURL=localAdminBootstrap.js.map
