...
void describe("D7 gate: end-to-end send→generate→sign→case creation", () => {
  void test("fixture customer walks full BMV intake then transitions to case", async () => {
    ...
    function e2eQueryFn(sql, params) {
    ...
    const gateInput = {
      caseTypeCode: "business_manager_visa",
      customerId: "c1",
      bmvQuestionnaireStatus: currentProfile.questionnaireStatus,
      bmvQuoteStatus: currentProfile.quoteStatus,
      bmvSignStatus: currentProfile.signStatus,
      bmvIntakeStatus: currentProfile.intakeStatus,
    };
...
void describe("D7 gate: BMV subtype parity (biz_mgmt_4m / biz_mgmt_renewal)", () => {
  const BMV_SUBTYPES = ["biz_mgmt_4m", "biz_mgmt_renewal"];
...
//# sourceMappingURL=customers.bmv.d7-gates.focused.test.js.map
