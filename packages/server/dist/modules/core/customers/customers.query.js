...
function buildCustomerCollaboratorExistsClause(alias, userParamRef) {
...
function buildActiveCasesExistsClause(alias) {
  return `exists (
    select 1
    from cases ca
...
function buildArchivedCasesExpr(customerAlias) {
  return `(select count(*)::int
    from cases ca
    where ${buildCaseBaseWhere("ca", customerAlias)}
      and (
...
function buildCaseNamesExpr(customerAlias) {
...
function buildLastCaseCreatedDateExpr(customerAlias) {
 ...
 */
function buildOwnerNameExpr(customerAlias) {
...
function appendKeywordWhere(alias, keyword, where, params) {
  if (!keyword) return;
...
function appendPhoneWhere(alias, phone, where, params) {
  if (!phone) return;
  const phoneRef = pushParam(params, `%${escapeLikePattern(phone)}%`);
  where.push(buildPhoneContainsClause(alias, phoneRef));
}
function appendJsonbFieldEqualsWhere(alias, value, fields, where, params) {
  if (!value) return;
...
function appendJsonbFieldContainsWhere(alias, value, fields, where, params) {
  if (!value) return;
...
function appendActiveCasesWhere(alias, activeCases, where) {
  if (activeCases === "yes") {
...
function buildDuplicateNameClause(nameRef) {
 ...
 */
export function buildCustomerListWhere(ctx, input) {
  const alias = "c";
 ...
 */
export function buildCustomerListSelect(alias = "c") {
  return [
    `${alias}.*`,
 ...
 */
export function buildCustomerDetailSelect(alias = "c") {
 ...
 */
export function buildCustomerDuplicateWhere(input, params) {
  const where = [];
