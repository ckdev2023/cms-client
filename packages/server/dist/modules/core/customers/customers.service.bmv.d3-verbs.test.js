...
function makeBaseCustomerRow(overrides = {}) {
  return {
    id: "c1",
    org_id: ctx.orgId,
    type: "individual",
...
void test("saveBmvSurvey updates intake_form and patches questionnaireStatus to returned", async () => {
  ...
  assert.equal(timelineWrites[0].action, "customer.bmv_survey_saved");
});
...
void test("modifyBmvQuote creates new intake_form and updates currentQuoteFormId", async () => {
  ...
  assert.equal(timelineWrites[0].action, "customer.bmv_quote_modified");
  assert.equal(timelineWrites[0].payload.previousQuoteFormId, "old-quote-form");
});
...
void test("transitionBmvToCase creates case when signed with all gates passed", async () => {
  ...
  assert.equal(timelineWrites[0].action, "customer.bmv_transitioned_to_case");
});
...
//# sourceMappingURL=customers.service.bmv.d3-verbs.test.js.map
