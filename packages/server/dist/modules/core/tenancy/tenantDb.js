 ...
 * @returns void
 */
async function setOrgId(client, orgId) {
 ...
 * @returns void
 */
async function setActorUserId(client, actorUserId) {
 ...
 * @returns Drizzle DB
 */
function createDrizzleDb(client) {
 ...
 */
function createTenantDrizzleRepositorySession(client, allowedAssertTables) {
  const db = createDrizzleDb(client);
  return {
    db,
    async assertBelongsToOrg(table, id) {
 ...
 * @returns fn 结果
 */
async function withTenantClient(pool, orgId, actorUserId, fn) {
 ...
 * @returns TenantDb
 */
export function createTenantDb(pool, orgId, actorUserId) {
  if (!isUuid(orgId)) {
 ...
 */
export function createTenantDrizzleDb(pool, orgId, actorUserId) {
  if (!isUuid(orgId)) {
    throw new Error("Invalid orgId (uuid required)");
  }
  if (actorUserId && !isUuid(actorUserId)) {
