...
import type {
  ...
  LeadBmvConvertContext,
  LeadConvertToCasePayload,
} from "./leads.types";
...
void describe("leads type contracts", () => {
  void test("LeadBmvConvertContext can be constructed", () => {
    const ctx: LeadBmvConvertContext = {
      customerId: "cust-1",
  ...
  void test("LeadConvertToCasePayload includes sourceChannel and leadId", () => {
    const payload: LeadConvertToCasePayload = {
      customerId: "cust-1",
      caseTypeCode: "business_manager_visa",
      ownerUserId: "user-1",
      orgId: "org-1",
  ...
  void test("LeadBmvReadinessSnapshot has all 8 boolean fields", () => {
    const snap: LeadBmvReadinessSnapshot = {
      isAssigned: true,
    ...
    for (const k of keys) {
      assert.equal(typeof snap[k as keyof LeadBmvReadinessSnapshot], "boolean");
    }
  });
});
