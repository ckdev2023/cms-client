 ...
 *
 */
export type ExpiryRiskSuggestionCode =
  (typeof EXPIRY_RISK_SUGGESTION_CODES)[number];

/**
 *
 */
export type ExpiryRiskStatus = "expired" | "expiring_soon" | "valid" | "none";

/**
 *
 */
export type AffectedCaseRow = {
  case_id: string;
  case_no: string | null;
 ...
 *
 */
export type SharedExpiryRiskResult = {
  assetId: string;
  latestVersionExpiryDate: string | null;
  riskStatus: ExpiryRiskStatus;
  daysUntilExpiry: number | null;
  suggestions: ExpiryRiskSuggestionCode[];
 ...
 */
export function computeExpiryRisk(expiryDateStr: string | null): {
  riskStatus: ExpiryRiskStatus;
  daysUntilExpiry: number | null;
  suggestions: ExpiryRiskSuggestionCode[];
} {
 ...
 */
export function mapAffectedCaseRow(row: AffectedCaseRow): AffectedCase {
  return {
    caseId: row.case_id,
    caseNo: row.case_no,
...
/** 数据库查询返回的 document_assets 行类型（含派生列）。 */
export type DocumentAssetQueryRow = {
  id: string;
  org_id: string;
  material_code: string;
  owner_subject_type: string;
  owner_customer_id: string | null;
  owner_employer_identity_key: string | null;
  origin_case_id: string | null;
  source_requirement_id: string | null;
  active_flag: boolean;
  created_at: unknown;
  updated_at: unknown;
  latest_version_expiry_date: unknown;
  referenced_case_count: unknown;
};

/** DocumentAsset 实体（含派生字段）。 */
export type DocumentAsset = {
  ...
  updatedAt: string;
  latestVersionExpiryDate: string | null;
  referencedCaseCount: number;
  isExpired: boolean;
};

/** 资产列表查询参数。 */
export type DocumentAssetListInput = {
  caseId?: string;
  materialCode?: string;
 ...
 */
export function mapDocumentAssetRow(row: DocumentAssetQueryRow): DocumentAsset {
 ...
 */
export function buildAssetListFilters(
  input: DocumentAssetListInput,
): ListWhereResult {
  const where: string[] = ["da.active_flag = true"];
