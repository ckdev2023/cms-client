...
import type { DocumentAssetQueryRow } from "./documentAssets.shared";
...
function makeAssetRow(): DocumentAssetQueryRow {
  return {
    id: ASSET_ID,
    org_id: ORG_A,
...
void test("DocumentAssetsService.list sets org_id via set_config for tenant isolation", async () => {
  ...
  const svc = new DocumentAssetsService(pool as unknown as Pool);
