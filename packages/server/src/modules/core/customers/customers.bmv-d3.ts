...
type BmvTransitionDeps = BmvActionDeps & {
  createCase: (ctx: RequestContext, input: CaseCreateInput) => Promise<Case>;
};

function requireCustomer(entity: Customer | null): asserts entity is Customer {
 ...
 * @returns 更新后的客户实体。
 */
export async function saveBmvSurvey(
  deps: BmvActionDeps,
 ...
 * @returns 更新后的客户实体。
 */
export async function modifyBmvQuote(
  deps: BmvActionDeps,
  ...
  const tenantDb = createTenantDb(pool, ctx.orgId, ctx.userId);
  const insertResult = await tenantDb.query<{ id: string }>(
...
function resolveOwnerAndGroup(
  input: TransitionBmvToCaseInput | undefined,
  lead: LeadInheritance | null,
  fallbackUserId: string,
): { ownerUserId: string; groupId: string | null } {
 ...
 */
async function resolveTransitionInputs(
 ...
 * @returns 创建的案件実体。
 */
export async function transitionBmvToCase(
  deps: BmvTransitionDeps,
 ...
 * @returns BMV 聚合 DTO。
 */
export async function getBmvAggregate(
  deps: Omit<BmvActionDeps, "timelineService">,
...
function assertBmvTransitionReady(
  profile: CustomerBmvProfile,
  customerId: string,
): void {
  if (profile.signStatus !== "signed") {
...
async function projectSurveyData(
  tenantDb: TenantDb,
  customerId: string,
...
async function fetchBmvCase(
  tenantDb: TenantDb,
  customerId: string,
): Promise<BmvCaseSummary | null> {
  const result = await tenantDb.query<{
    id: string;
    stage: string | null;
    post_approval_stage: string | null;
    coe_issued_at: string | null;
    coe_expiry_date: string | null;
    coe_sent_at: string | null;
    status: string;
  }>(
...
async function fetchReminders(
  tenantDb: TenantDb,
  caseId: string,
): Promise<BmvReminderSummaryItem[]> {
  const result = await tenantDb.query<{
    id: string;
    remind_at: string;
    send_status: string;
