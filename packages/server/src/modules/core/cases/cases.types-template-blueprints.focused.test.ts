...
void describe("validateExtraFieldsSchema", () => {
  ...
  void test("rejects non-array", () => {
    assert.equal(
      validateExtraFieldsSchema("nope" as unknown as unknown[]),
      false,
    );
  });
  ...
  void test("rejects missing fieldKey", () => {
    const rest = { ...VALID_FIELD } as Record<string, unknown>;
...
void describe("validateRequirementBlueprint", () => {
  ...
  void test("rejects non-array", () => {
    assert.equal(
      validateRequirementBlueprint({} as unknown as unknown[]),
      false,
    );
  });
  ...
  void test("rejects missing checklistItemCode", () => {
    const rest = { ...VALID_REQ } as Record<string, unknown>;
...
void describe("validateWorkflowStepsBlueprint", () => {
  ...
  void test("rejects non-array", () => {
    assert.equal(
      validateWorkflowStepsBlueprint(42 as unknown as unknown[]),
      false,
    );
  });
  ...
  void test("rejects missing stepCode", () => {
    const rest = { ...VALID_STEP } as Record<string, unknown>;
...
void describe("parseCaseTemplateConfig", () => {
  ...
  void test("round-trips through JSON serialization", () => {
    ...
    const serialized = JSON.parse(JSON.stringify(config)) as Record<
      string,
      unknown
    >;
