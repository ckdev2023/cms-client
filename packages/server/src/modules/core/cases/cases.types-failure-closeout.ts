 ...
 *
 */
export type FailureCloseoutReasonCode =
  (typeof FAILURE_CLOSEOUT_REASON_CODES)[keyof typeof FAILURE_CLOSEOUT_REASON_CODES];

// ─── resultOutcome → 帰因コード マッピング ──────────────────

const OUTCOME_TO_REASON: Readonly<
  Record<string, FailureCloseoutReasonCode | undefined>
> = {
...
export const FAILURE_OUTCOME_SET: ReadonlySet<string> = new Set(
  Object.keys(OUTCOME_TO_REASON),
);
...
export const FAILURE_CLOSEOUT_PATHS = {
  VISA_REJECTED: {
    ...
    resultOutcomeValues: ["visa_rejected", "rejected"] as readonly string[],
    canDirectClose: true,
    closeReasonRequired: false,
    adminGuidance: "prompt_transition_to_S9" as const,
  },
  APPLICATION_REJECTED: {
    ...
    resultOutcomeValues: ["rejected"] as readonly string[],
    canDirectClose: true,
    closeReasonRequired: false,
    adminGuidance: "prompt_transition_to_S9" as const,
  },
  CLIENT_WITHDRAWN: {
    ...
    resultOutcomeValues: ["withdrawn"] as readonly string[],
    canDirectClose: true,
    closeReasonRequired: false,
    adminGuidance: "prompt_transition_to_S9" as const,
  },
  MANUAL_FAILURE_CLOSE: {
    ...
    resultOutcomeValues: [] as readonly string[],
    canDirectClose: true,
    closeReasonRequired: true,
    adminGuidance: "require_close_reason_input" as const,
  },
} as const;
 ...
 *
 */
export type FailureCloseoutAttribution = {
  reasonCode: FailureCloseoutReasonCode;
  reasonLabel: string;
...
/**
 * 失敗結案チェック結果 — CaseDetailAggregateDto に埋め込む。
 *
 * isFailurePath = true → admin は「失敗結案」ボタンを表示可能。
 * attribution が null → 帰因が未確定（操作者に closeReason or resultOutcome 入力を促す）。
 */
export type FailureCloseoutCheckResult = {
  isFailurePath: boolean;
  attribution: FailureCloseoutAttribution | null;
};
 ...
 *
 */
export type FailureCloseoutCheckInput = {
  caseEntity: Case;
  closeReason?: string | null;
};

function resolveOutcomeAttribution(
  resultOutcome: string | null,
): FailureCloseoutAttribution | null {
  if (resultOutcome === null) return null;
  const reasonCode: FailureCloseoutReasonCode | undefined =
    OUTCOME_TO_REASON[resultOutcome];
...
export function resolveFailureAttribution(
  input: FailureCloseoutCheckInput,
): FailureCloseoutAttribution | null {
 ...
 * @param caseEntity
 */
export function checkFailureCloseout(
  caseEntity: Case,
 ...
 * @param closeReason
 */
export function canBypassSuccessCloseoutForFailure(
  caseEntity: Case,
  closeReason?: string | null,
): boolean {
 ...
 *
 */
export type FailureCloseoutErrorCode =
  (typeof FAILURE_CLOSEOUT_ERROR_CODES)[keyof typeof FAILURE_CLOSEOUT_ERROR_CODES];
