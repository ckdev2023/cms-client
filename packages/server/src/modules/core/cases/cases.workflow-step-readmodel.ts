...
import type { WorkflowStepSummary } from "./cases.types-workflow-step";
import {
  isBmvWorkflowStep,
...
const BMV_STEP_LOOKUP: ReadonlyMap<
  string,
  (typeof BMV_WORKFLOW_STEPS_BLUEPRINT)[number]
> = new Map(BMV_WORKFLOW_STEPS_BLUEPRINT.map((s) => [s.stepCode, s]));
 ...
 */
export function resolveWorkflowStepSummary(
  caseEntity: Case,
