...
import type { Reminder, Task } from "../model/coreEntities";
import type {
  CaseDeadlineDto,
...
void test("CaseTaskListResult wraps items and total", () => {
  const result: CaseTaskListResult = { items: [], total: 0 };
  assert.equal(result.items.length, 0);
  assert.equal(result.total, 0);
});

void test("CaseTaskListInput requires caseId, other fields are optional", () => {
  const minimal: CaseTaskListInput = { caseId: "c-1" };
  assert.equal(minimal.caseId, "c-1");

  const full: CaseTaskListInput = {
    caseId: "c-1",
    status: "pending",
    assigneeUserId: "u-1",
    page: 1,
...
void test("CaseDeadlineListResult wraps items", () => {
  const result: CaseDeadlineListResult = { items: [] };
  assert.equal(result.items.length, 0);
});

void test("CaseDeadlineSourceFields maps to Case entity date fields", () => {
  const source: CaseDeadlineSourceFields = {
    dueAt: "2026-04-30T00:00:00Z",
...
void test("CaseReminderListResult wraps items and total", () => {
  const result: CaseReminderListResult = { items: [], total: 0 };
  assert.equal(result.items.length, 0);
  assert.equal(result.total, 0);
});
