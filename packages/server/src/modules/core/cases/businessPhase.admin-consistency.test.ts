...
} = (await import(adminTransitionsPath)) as {
  PHASE_TRANSITIONS: Readonly<Record<string, readonly string[]>>;
  isTerminalPhase: (phase: string) => boolean;
};

const { BUSINESS_PHASES: ADMIN_PHASES } = (await import(adminPhasesPath)) as {
  BUSINESS_PHASES: readonly string[];
};
...
void describe("admin↔server PHASE_TRANSITIONS consistency (BUG-208)", () => {
  ...
  void test("admin PHASE_TRANSITIONS has no extra keys beyond server phases", () => {
    const serverSet = new Set<string>(SERVER_PHASES);
