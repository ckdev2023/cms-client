...
void describe("§14 supplement cycle: step transition rules (pure)", () => {
  void test("supplement cycle is contained within S5", () => {
    const cycleSteps: BmvWorkflowStep[] = [
      "UNDER_REVIEW",
      "NEED_SUPPLEMENT",
...
void describe("§15 supplement_count recalc via service (BUG-118)", () => {
  ...
  void test("mapCaseRow maps supplement_count from numeric string", () => {
    ...
    const c = mapCaseRow(row as unknown as CaseQueryRow);
...
void describe("§17 mapCaseRow: P1 fields round-trip", () => {
  void test("maps visa_plan string", () => {
    const c = mapCaseRow(
      makeCaseRow({ visa_plan: "new_3year" }) as unknown as CaseQueryRow,
    );
  ...
  void test("maps visa_plan null", () => {
    const c = mapCaseRow(
      makeCaseRow({ visa_plan: null }) as unknown as CaseQueryRow,
    );
  ...
  void test("maps quote_price from numeric string", () => {
    const c = mapCaseRow(
      makeCaseRow({ quote_price: "350000.50" }) as unknown as CaseQueryRow,
    );
  ...
  void test("maps quote_price null", () => {
    const c = mapCaseRow(
      makeCaseRow({ quote_price: null }) as unknown as CaseQueryRow,
    );
  ...
  void test("maps supplement_count from DB string", () => {
    const c = mapCaseRow(
      makeCaseRow({ supplement_count: "5" }) as unknown as CaseQueryRow,
    );
