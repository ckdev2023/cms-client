...
const mockAggregate: CaseBillingTabAggregate = {
  summary: {
    quotePrice: 500000,
    totalDue: 300000,
...
void test("getBillingTabAggregate throws when case not found", async () => {
  ...
  await assert.rejects(
    () => controller.getBillingTabAggregate(viewerCtxReq as never, "missing"),
    NotFoundException,
  );
});
...
void test("getBillingTabAggregate: missing requestContext throws UnauthorizedException", async () => {
  ...
  await assert.rejects(
    () => controller.getBillingTabAggregate({} as never, "case-1"),
    UnauthorizedException,
  );
});

void test("getBillingTabAggregate: empty plans and payments", async () => {
  const emptyAggregate: CaseBillingTabAggregate = {
    summary: {
      quotePrice: null,
      totalDue: 0,
      totalReceived: 0,
