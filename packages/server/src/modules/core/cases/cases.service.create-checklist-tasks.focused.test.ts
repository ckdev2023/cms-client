...
function makeResolver(response: unknown): {
      ...
      resolve: (_ctx: unknown, input: { kind: string; key: string }) => {
        ...
        return Promise.resolve(
          typeof response === "function"
            ? (response as (i: { kind: string; key: string }) => unknown)(input)
            : response,
        );
      },
...
function makeKindAwareResolver(responses: Record<string, unknown>): {
  service: TemplatesResolver;
  calls: { kind: string; key: string }[];
} {
  const calls: { kind: string; key: string }[] = [];
      ...
      resolve: (_ctx: unknown, input: { kind: string; key: string }) => {
        ...
        return Promise.resolve(
          typeof resp === "function"
            ? (resp as (i: { kind: string; key: string }) => unknown)(input)
