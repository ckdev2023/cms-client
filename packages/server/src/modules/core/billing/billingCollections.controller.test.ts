...
function makeReq(role: RequestContext["role"] = "staff") {
  return {
    requestContext: { orgId: ORG_ID, userId: USER_ID, role } as RequestContext,
  };
}
...
function makeService() {
  ...
  return {
    svc: {
      bulkCollect: (ctx: unknown, caseIds: unknown) => {
...
void test("controller: rejects empty caseIds", async () => {
  ...
  const ctrl = new BillingCollectionsController(svc as never);
...
void test("controller: accepts exactly 200 caseIds", async () => {
  ...
  assert.equal(
    ((calls[0] as Record<string, unknown>).caseIds as string[]).length,
    200,
  );
});
