...
import {
  ...
  mapRefRow,
  mapCandidateRow,
  type DocumentRequirementFileRef,
  type DocumentRequirementFileRefQueryRow,
  type LinkRefInput,
  type ReferenceCandidate,
  type ReferenceCandidateRow,
} from "./documentRequirementFileRefs.shared";

export type { LinkRefInput } from "./documentRequirementFileRefs.shared";

const LINK_TRANSITION_TARGET: DocumentItemStatusEnum = "uploaded_reviewing";

const LINK_TRANSITION_ELIGIBLE: readonly string[] = (
...
@Injectable()
export class DocumentRequirementFileRefsService {
  /**
   * 构造函数。
  ...
  async link(
    ctx: RequestContext,
    input: LinkRefInput,
  ): Promise<DocumentRequirementFileRef> {
   ...
   * @param refId 引用记录 ID
   */
  async unlink(ctx: RequestContext, refId: string): Promise<void> {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
  ...
  async get(
    ctx: RequestContext,
    id: string,
  ): Promise<DocumentRequirementFileRef | null> {
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
  ...
  async listByRequirement(
    ...
    const tenantDb = createTenantDb(this.pool, ctx.orgId, ctx.userId);
    const result = await tenantDb.query<DocumentRequirementFileRefQueryRow>(
      `SELECT ${REF_COLS}
  ...
  async listCandidates(
    ...
    const result = await tenantDb.query<ReferenceCandidateRow>(
      `SELECT
         df.id,
         df.requirement_id,
         df.file_name,
  ...
  private async insertRef(
    tx: TenantDbTx,
    ctx: RequestContext,
    input: LinkRefInput,
  ): Promise<DocumentRequirementFileRef> {
    const result = await tx.query<DocumentRequirementFileRefQueryRow>(
  ...
  private async writePostLinkTimelines(
    ctx: RequestContext,
    ref: DocumentRequirementFileRef,
    input: LinkRefInput,
  ...
  private async assertRequirementExists(
    tx: TenantDbTx,
    requirementId: string,
  ): Promise<{ status: string; caseId: string }> {
  ...
  private async assertFileVersionExists(
    tx: TenantDbTx,
  ...
  private async transitionItemAfterLink(
    tx: TenantDbTx,
    requirementId: string,
