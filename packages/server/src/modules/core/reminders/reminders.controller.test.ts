...
function makeController(
  overrides: Partial<Record<keyof RemindersService, unknown>> = {},
) {
  const service = {
...
void test("RemindersController.get requires context and handles not found", async () => {
  ...
  await assert.rejects(
    () => controller.get({} as never, REMINDER_ID),
    UnauthorizedException,
  );
  await assert.rejects(
  ...
  const res = await controller.get(req as never, REMINDER_ID);
  assert.equal(res.id, REMINDER_ID);
});
