...
type QFn = (sql: string, params?: unknown[]) => Promise<QR>;

function ctx(): RequestContext {
  return { orgId: ORG, userId: USER, role: "staff" };
}

function svc(qfn: QFn, stage = "S7") {
  const pool = {
  ...
  return new SubmissionPackagesService(
    ...
    { write: () => Promise.resolve() } as never,
    {
      ...
      transition: (_: unknown, id: string) =>
        Promise.resolve({ id, stage: "S7", status: "S7" }),
...
function pkgRow(overrides: Record<string, unknown> = {}) {
  return {
    id: PKG_INIT,
    org_id: ORG,
    case_id: CASE,
...
function supInput(relatedId: string) {
  return {
    caseId: CASE,
    submissionKind: "supplement" as const,
    relatedSubmissionId: relatedId,
    authorityName: "Tokyo Immigration",
    submittedAt: "2026-04-01T10:00:00.000Z",
    items: [
      {
        itemType: "field_snapshot" as const,
        refId: REQ_ID,
...
function baseRoutes(
  latestPkgId: string,
  existingPkgId: string,
  opts: {
    branchChild?: string | null;
    chainLinks?: Map<string, string | null>;
  } = {},
): QFn {
  return (sql, params) => {
