...
function makeController(
  opts: { service?: Partial<DocumentItemsService> } = {},
...
void test("getCompletionRate: forwards caseId to service and returns result", async () => {
  let calledCaseId: string | undefined;
  const controller = makeController({
    service: {
      getCompletionRate: (_ctx: unknown, caseId: string) => {
        calledCaseId = caseId;
  ...
  const result = await controller.getCompletionRate(req as never, {
    caseId: CASE_ID,
  });
...
void test("getCompletionRate: throws UnauthorizedException without context", async () => {
  ...
  await assert.rejects(
    () => controller.getCompletionRate({} as never, { caseId: CASE_ID }),
...
void test("getCompletionRate: throws BadRequestException when caseId is missing", async () => {
  const controller = makeController();
  await assert.rejects(
    () => controller.getCompletionRate(req as never, {}),
    BadRequestException,
  );
});
...
void test("getCompletionRate: throws BadRequestException when caseId is empty string", async () => {
  const controller = makeController();
  await assert.rejects(
    () => controller.getCompletionRate(req as never, { caseId: "" }),
    BadRequestException,
  );
});
