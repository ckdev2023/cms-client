...
| ~~P0-Critical~~ | ~~`DocumentAsset`~~              | §3.9A   | ✅ 已修复——见 §8 条目 3                    |
| ~~P0-Critical~~ | ~~`DocumentRequirementFileRef`~~ | §3.10A  | ✅ 已修复——见 §8 条目 3                    |
...
| 变更                                                                               | 类型     | 说明                                                                        |
| ---------------------------------------------------------------------------------- | -------- | --------------------------------------------------------------------------- |
| ~~创建 `document_assets` 表~~                                                      | DDL      | ✅ 已落地（migration 017/019 + D1 Service/Controller）——见 §8 条目 3        |
| ~~创建 `document_requirement_file_refs` 表~~                                       | DDL      | ✅ 已落地（migration 017/019 + D2 Service/Controller）——见 §8 条目 3        |
...
| ~~资料四层模型仅有两层~~           | ~~🟠 High~~ | ✅ 已修复——DDL(017/019) + Service/Controller(D1/D2) 已落地；见 §8 条目 3 |
...
> 以下缺口已在资料中心 P0 闭环 + P1 四层模型计划中修复。

| #   | 缺口                                     | 修复阶段            | 修复内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --- | ---------------------------------------- | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `document_items` 缺 waive 原因持久化     | A1 / A4             | 新增 `POST /api/document-items/:id/waive` 端点，写入 `waive_reason_code_latest` / `waive_reason_latest` / `waived_by_user_id_latest` / `waived_at_latest`；A1.b 同步关闭 transition→waived 路径，强制走专用端点；A4 在 list/get 返回中补齐这四个字段。                                                                                                                                                                                                                      |
| 2   | `document_files` upload 未联动 item 状态 | A3                  | `document-files.upload` 在 `relative_path` 模式成功插入版本后，同事务内将所属 `document_items.status` 由 `pending`/`waiting_upload`/`revision_required` → `uploaded_reviewing`；`DOCUMENT_ITEM_ALLOWED_TRANSITIONS` 已扩展 `pending→uploaded_reviewing` 与 `revision_required→uploaded_reviewing` 两条边；S9 readonly 守卫已补齐。                                                                                                                                          |
| 3   | Asset / RequirementFileRef 服务层缺失    | D1 / D1.b / D2 / D3 | DDL 已在 migration 017/019 落地（含 RLS）；D1 新增 `DocumentAssetsController` + `Service`（只读，写路径由 upload 内部触发）；D1.b 补唯一索引 `(org_id, material_code, owner_subject_type, owner_customer_id)` + 对应 `owner_employer_identity_key`（migration 036）；D2 新增 `DocumentRequirementFileRefsController` + `Service`（link / unlink / candidates，含 submission package 锁定守卫）；D3 在 upload 事务内 upsert asset + 写 `ref_mode=direct_register` 引用记录。 |
