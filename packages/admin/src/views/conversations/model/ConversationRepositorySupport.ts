...
interface ConversationRepositoryErrorInput {
 ...
 *
 */
export interface ConversationRepositoryFactoryInput {
  /**
   *
   */
 ...
 *
 */
export interface ConversationRepositoryRuntime {
  /**
   *
   */
  request: typeof fetch;
...
export class ConversationRepositoryError extends Error {
   ...
   * @param input - 结构化错误描述
   */
  constructor(input: ConversationRepositoryErrorInput) {
...
function buildBadResponseError(
  ...
  const code: ConversationRepositoryErrorCode =
    response.status === 401
      ? "UNAUTHORIZED"
...
export async function requestAndAdapt<T>(input: {
  runtime: ConversationRepositoryRuntime;
  url: string;
  method: "GET" | "POST" | "PATCH" | "DELETE";
  body?: unknown;
