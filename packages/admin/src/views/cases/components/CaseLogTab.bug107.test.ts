...
describe.skip("CaseLogTab BUG-107: timestamps must not leak Date.toString()", () => {
  ...
  it("Date.toString() leaked from /api/timeline (BUG-087) is normalized for display", () => {
    const raw = "Wed Apr 29 2026 11:32:06 GMT+0900 (Japan Standard Time)";
  ...
  it("falls back to the raw string when value is unparseable (no silent blank)", () => {
    const w = mountTab([buildEntry({ time: "not-a-date" })], "ja-JP");
    const text = readTimeCells(w.html())[0];
    expect(text).toBe("not-a-date");
  });

  it("renders empty cell for empty time field instead of throwing", () => {
