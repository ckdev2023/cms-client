const billingZhCN = {
  list: {
    title: "收费与财务",
    registerPayment: "登记回款",
    summaryLabel: "收费摘要",
    segments: {
      billingList: "案件收费列表",
      paymentLog: "回款流水记录",
    },
    segmentAriaLabel: "列表视图切换",
    searchPlaceholder: "搜索：案件名称 / 客户名称 / 案件编号",
    filters: {
      statusAll: "所有回款状态",
      statusAriaLabel: "筛选回款状态",
      groupAll: "所有分组",
      groupAriaLabel: "筛选所属分组",
      ownerAll: "所有负责人",
      ownerAriaLabel: "筛选负责人",
      reset: "重置",
      recordCount: "共 {count} 条记录",
    },
    summary: {
      totalDue: "总应收 (JPY)",
      totalReceived: "总已收 (JPY)",
      totalOutstanding: "总待收金额 (JPY)",
      overdueAmount: "逾期欠款 (JPY)",
    },
    columns: {
      selectAll: "全选案件",
      selectRow: "选择 {name}",
      caseName: "案件名称",
      client: "客户",
      group: "所属分组",
      amountDue: "应收(¥)",
      amountReceived: "已收(¥)",
      amountOutstanding: "待收 (¥)",
      nextNode: "应收摘要",
      status: "回款状态",
      actions: "操作",
    },
    actions: {
      registerPayment: "登记回款",
    },
    status: {
      overdue: "逾期",
      partial: "部分回款",
      due: "未回款",
      paid: "已结清",
    },
    groups: {
      "tokyo-1": "東京一組",
      "tokyo-2": "東京二組",
      osaka: "大阪組",
    },
    nodeEmpty: "无（已结清）",
    empty: {
      title: "暂无收费记录",
      description: "请先在案件中配置收费计划",
      noData: "暂无收费记录",
      noResultForFilters: "当前筛选条件下暂无结果",
    },
    bulk: {
      ariaLabel: "批量操作",
      selected: "已选择",
      count: "{count}",
      unit: "条",
      clear: "清除",
      collect: "批量生成催款任务",
      emptyHint: "当前没有逾期应收，无法发起批量催款",
    },
    pagination: {
      summary: "显示 {start} - {end} 条，共 {total} 条",
      prev: "上一页",
      next: "下一页",
    },
    toast: {
      bulkCollect: {
        title: "催款任务已生成",
        description: "成功 {success} 件、跳过 {skipped} 件、失败 {failed} 件",
      },
      paymentRegistered: {
        title: "回款登记成功",
        description: "回款已成功登记。",
      },
      voided: {
        title: "已作废",
        description: "流水 {id} 已标记为作废",
      },
      reversed: {
        title: "已冲正",
        description: "流水 {id} 已标记为冲正",
      },
    },
    error: {
      loadFailed: "数据加载失败，请重试。",
      retry: "重试",
    },
  },
  bulkCollect: {
    skipReason: {
      "no-permission": "无权限编辑该案件",
      "case-not-found": "案件不存在或已删除",
      "duplicate-task": "已存在未完成催款任务",
      "not-overdue": "无逾期收费计划",
      "no-assignee": "案件未分配负责人",
      "system-error": "系统错误",
    },
    drawer: {
      title: "催款任务执行结果",
      empty: "暂无明细",
      details: "明细",
      successLabel: "成功",
      skippedLabel: "跳过",
      failedLabel: "失败",
      unknownCase: "案件信息缺失",
    },
  },
  paymentLog: {
    columns: {
      amount: "回款金额(¥)",
      date: "回款日期",
      caseName: "关联案件",
      node: "关联收费计划",
      receipt: "凭证",
      recordStatus: "记录状态",
      operator: "操作人",
      note: "备注",
      actions: "操作",
    },
    receiptYes: "有",
    receiptNo: "无",
    recordStatus: {
      valid: "有效",
      voided: "已作废",
      reversed: "已冲正",
    },
    actionVoid: "作废",
    actionReverse: "冲正",
    toast: {
      voided: {
        title: "已作废",
        description: "流水 {id} 已标记为作废",
      },
      reversed: {
        title: "已冲正",
        description: "流水 {id} 已标记为冲正",
      },
    },
    row: {
      voidedBy: "{name} 作废",
      reversedBy: "{name} 冲正",
    },
    empty: {
      title: "暂无回款流水",
      description: "登记回款后，流水记录将显示在此处",
    },
  },
  riskAck: {
    modal: {
      title: "收费风险确认",
      submit: "确认",
      cancel: "取消",
    },
    reasonCode: {
      customer_promise: "客户承诺一周内补缴",
      internal_review: "事务所内部审查通过",
      partial_settled: "部分尾款已收，余款风险可控",
      other: "其它",
    },
    reasonNote: {
      placeholder: '请描述原因（选择"其它"时必填）',
    },
    evidenceUrl: {
      placeholder: "证据链接（可选）",
    },
    chip: {
      acknowledged: "已风险确认（{date}）",
      notAcknowledged: "未风险确认",
    },
  },
  paymentModal: {
    title: "登记回款",
    closeAriaLabel: "关闭",
    hint: "请填写回款信息。默认每案件一笔一次性应收。金额与日期为必填项。",
    fields: {
      amount: "金额 (¥)",
      amountPlaceholder: "输入回款金额",
      amountWarning: "金额超过该笔应收未收余额，建议拆分为多笔登记或核对总额",
      date: "日期",
      node: "关联收费计划",
      nodePlaceholder: "请选择收费计划",
      nodeError: "存在多笔未结清应收，请先选择对应收费计划",
      receipt: "付款凭证",
      receiptPlaceholder: "凭证编号或文件名（可后补）",
      receiptHint: "支持图片或 PDF，可后补",
      note: "备注",
      notePlaceholder: "可选：补充说明",
    },
    cancel: "取消",
    submit: "登记回款",
    submitting: "提交中…",
    loadingNodes: "正在加载收费信息…",
    submitError: "回款登记失败，请重试。",
  },
  milestone: {
    down_payment: "着手金",
    final_payment: "尾款",
    balance: "残金",
    interim_payment: "中间金",
    installment: "分期付款",
    case_fee: "案件报酬",
  },
} as const;

export default billingZhCN;
